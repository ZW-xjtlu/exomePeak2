...
#' @param logFC_mod a non negative numeric value of the log2 fold change cutoff used in DESeq inferene for modification peaks (IP > input).
...
#' @return a list of the index for the significant modification containing peaks (IP > input) and control peaks (peaks other than modification containing peaks).
#'
...
DESeq_inference <- function(count_assay,
                            ...
                            count_cutoff = 10,
                            logFC_mod = 0,
                            min_mod_number = floor( nrow(count_assay)*0.002 )) {
  ...
  decision_table <- decision_deseq(res = res,
                                   log2FC_cut = logFC_mod,
                                   p_cut =  p_cutoff,
                                   padj_cut = p_adj_cutoff,
                                   min_mod =  min_mod_number)
  ...
  sig_peak_mod <- as.numeric( rownames(res)[stat_sig_indx & res$log2FoldChange > logFC_mod] )

  return(
  index_mod = indx_count[as.numeric( sig_peak_mod )]
  )
}
