...
By default, $t_{i,j}$ is further decomposed by the logarithm of the sample-specific size factor $s_j$ and the GC content bias offset $f(GC_i)$.  $\hat s_j$ is estimated using the median read count on the background bins, which are classified by GMM from the bin's POI values: (IP count + 1)/(input count + 1). $\hat f(GC_i)$ is estimated using splines expanded Poisson GLM on bin's read count with bin's fragment GC content $f(GC_i)$ as covariate. Knots for the smoothing splines used are set at `c(0, 0.4, 0.5, 0.6, 1)`. For identifiability, the fitted values $\hat f(GC_i)$ are centered at 1 by dividing the sample mean.
