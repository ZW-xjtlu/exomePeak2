...
$Q_{i,j}$ is the expected value of reads abundance of modification $i$ under sample $j$. ${\beta}_{i,0}$ is the intercept coefficient, ${\beta}_{i,1}$ is the coefficient for IP/input log2 fold change, $I(\rho(j)=IP)$ is the regression covariate that is the indicator variable for the sample $j$ being IP sample. $t_{i,j}$ is the regression offset that account for the sequencing depth variation and the GC content biases. $t{i,j}$ is estimated with the median regression on the logarithm of reads count implemented in the package **cqn**[1]; the median regression is smoothed using the natural cubic splines with the knots defined in function `cqn::cqn()`. By default, the quantile normalization functionality in **cqn** is turned off so that only the GC bias offsets are estimated from the read counts.

Under the default settings, the linear models fitted are the regularized **GLM (Generalized Linear Model)** of NB developed by **DESeq2**[2]. If one of the IP and input group has no biological replicates, Poisson GLMs will be fitted to the modification peaks. Both the size factors used in **cqn** and **DESeq2** are estimated by the robust size factor estimators defined in the function `DESeq2::estimateSizeFactors()` with type = "ratio".
