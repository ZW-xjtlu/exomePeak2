...
Under the default settings, the peak statistics are derived from the ${\beta}_{i,1}$ terms in the following regression design using the **GLM (Generalized Linear Model)** developed by **DESeq2**:

$$log2(Q_{i,j}) = {\beta}_{i,0} + {\beta}_{i,1}I(\rho(j)=IP) + t_{i,j}$$

Where $Q_{i,j}$ is the expected value of reads abundence of the modification peak $i$ under sample $j$. ${\beta}_{i,0}$ is the intercept coefficient, ${\beta}_{i,1}$ is the coefficient for IP/input log2 fold change, $I(\rho(j)=IP)$ is the regression covariate that is the indicator variable for the sample $j$ being IP sample. $t_{i,j}$ is the regression offset that account for the sequencing depth variation and the GC content biases.
...
The peak statistics in differential modification setting are derived from the interactive coefficient ${\beta}_{i,3}$ in the following regression design of the **DESeq2 GLM**:
